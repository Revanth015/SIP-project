...
if run_scenario:
    ...
    try:
        ...
        # Critical rule: only slots generated by Model 1 are eligible. Filter them by the selected area.
...
with tabs[1]:
    if not result:
        ...
        r = result; cap, sim = r["capacity"], r["simulation_summary"]
        ...
        st.pyplot(plot_layout(
            ...
            r["obstacles"], title="Model 1 · Generated Feasible Pallet Slots"
...
with tabs[2]:
    if not scenario:
        ...
        st.pyplot(plot_slot_allocation(
            result["warehouse"], scenario["region"],
            [result["winner"]["layout"]["slots"][i] for i in range(len(result["winner"]["layout"]["slots"])) if False],
            scenario["allocation"], result["door"],
            title=f"Model 2 · {scenario['strategy']} · Slot-by-Slot Allocation"
        ), use_container_width=True)
        # Rebuild the selected slot geometry in the same order as the filtered Model 1 slot master.
        selected_ids = set(scenario["slot_df"]["SLOT_ID"])
        selected_geoms = []
        for geom, row in zip(result["slots"], result["slot_df"].itertuples(index=False)):
...
with tabs[3]:
    ...
    if scenario:
        st.write(f"**Model 2:** the selected area contains **{scenario['summary']['total_slots']:,} Model 1-generated slots**. Under **{scenario['strategy']}**, the simulation allocates slots sequentially according to observed SKU movement characteristics.")
        st.info("This is a scenario simulation, not a claim that the selected SKU allocation is the current physical inventory arrangement. The belts-per-slot value is an explicit scenario conversion parameter and should be replaced with a validated warehouse capacity value when available.")
