...
1) MTO transactions, 2) MTA transactions, and 3) Size–Box Master.
"""
...
with st.sidebar:
    ...
    st.caption("Model 2 accepts exactly these three supplied files. It does not request any additional operational data file.")
    ...
    area_mode = st.selectbox(
        "Analysis area",
        ["Whole warehouse", "Left half", "Right half", "Lower half", "Upper half", "Custom rectangle"],
    )
    ...
    belts_per_slot = st.number_input(
        ...
        help="Planning parameter for converting movement quantity into slot equivalents. It is not treated as observed inventory capacity.",
    )
...
# -----------------------------------------------------------------------------
# Model 1: CAD -> feasible physical slot master
# -----------------------------------------------------------------------------
if cad_file:
...
# -----------------------------------------------------------------------------
# Model 2: exactly three supplied operational files + Model 1 slot master
# -----------------------------------------------------------------------------
if run_scenario:
    ...
    missing = []
    if not mto_file:
        missing.append("MTO Consolidated Master")
    if not mta_file:
        missing.append("MTA Consolidated Master")
    if not box_file:
        missing.append("Size–Box Master Table")
    if missing:
    ...
    try:
        ...
        # Critical integration rule: Model 2 NEVER creates its own physical slots.
        # It filters only the slot master generated by Model 1.
        slot_df = r["slot_df"].copy()
        slot_df = slot_df[
            slot_df.apply(
                lambda row: region.covers(Point(row["X_M"], row["Y_M"])),
                axis=1,
            )
        ].copy()
        ...
        with st.spinner("Preparing the three supplied Model 2 files and allocating Model 1 slots…"):
            sku, mto_clean, mta_clean, box_clean = prepare_movement(
                save_upload(mto_file),
                save_upload(mta_file),
                save_upload(box_file),
            )
            scenario_table, allocations = build_scenario_table(sku, slot_df, belts_per_slot)
            selected_rank = rank_skus(sku, strategy)
            selected_allocation, selected_summary = allocate_slots(
                selected_rank, slot_df, belts_per_slot
            )
        ...
        st.success(
            f"Model 2 completed using {len(slot_df):,} Model 1-generated slots in the selected area."
        )
    except Exception as exc:
...
with tabs[2]:
    if not scenario:
        ...
        st.caption(
            "Model 2 uses only the three supplied files: MTO Consolidated Master, "
            "MTA Consolidated Master, and Size–Box Master Table. Physical slots are "
            "taken directly from Model 1."
        )
        ...
        st.info(
            f"Packing standard coverage: {master_count:,} SKUs from the Size–Box Master; "
            f"{default_count:,} SKUs use the documented default of {DEFAULT_BELTS_PER_BOX} belts/box."
        )

        st.pyplot(
            plot_slot_allocation(
                result["warehouse"], scenario["region"], result["slots"],
                scenario["allocation"], result["door"],
                title=f"Model 2 · {scenario['strategy']} · Selected Area",
            ),
            use_container_width=True,
        )
...
with tabs[3]:
    ...
    st.markdown(
        "**Model 1:** establishes the physically feasible pallet-slot master from the CAD and physical constraints.  \n"
        "**Model 2:** selects an area, filters those exact Model 1 slots, ranks SKUs from MTO/MTA movement, "
        "uses the Size–Box Master where available, and applies a 25-belts/box planning default where no valid "
        "standard is available."
    )
    st.warning(
        "The 25 belts/box value is a modelling fallback, not an observed company standard. "
        "It should be validated during implementation."
    )
    if scenario:
        best = scenario["scenario_table"].sort_values("movement_coverage_pct", ascending=False).iloc[0]
        st.success(
            f"Within the selected Model 1 area, the highest movement coverage in the tested scenarios is "
            f"{best['STRATEGY']} at {best['movement_coverage_pct']:.1f}%."
        )
...
with tabs[4]:
    ...
    if result:
        st.download_button(
            "Download Model 1 slot master",
            result["slot_df"].to_csv(index=False).encode("utf-8"),
            "model1_slot_master.csv",
            "text/csv",
        )
        st.download_button(
            "Download Model 1 daily simulation",
            result["daily_simulation"].to_csv(index=False).encode("utf-8"),
            "model1_daily_simulation.csv",
            "text/csv",
        )
    if scenario:
        st.download_button(
            "Download Model 2 SKU movement master",
            scenario["sku"].to_csv(index=False).encode("utf-8"),
            "model2_sku_movement_master.csv",
            "text/csv",
        )
        st.download_button(
            "Download Model 2 scenario comparison",
            scenario["scenario_table"].to_csv(index=False).encode("utf-8"),
            "model2_scenario_comparison.csv",
            "text/csv",
        )
        st.download_button(
            "Download Model 2 selected allocation",
            scenario["allocation"].to_csv(index=False).encode("utf-8"),
            "model2_selected_allocation.csv",
            "text/csv",
        )
